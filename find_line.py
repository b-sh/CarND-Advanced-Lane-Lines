...
#binary_warped = np.int8(binary_warped)
...
#histogram = np.histogram(binary_warped[np.int(binary_warped.shape[0]/2):,:])
plt.plot(histogram)
plt.show()
...
leftx_base  = np.min(histogram[:midpoint].nonzero())
rightx_base = np.max(histogram[midpoint:].nonzero()) + midpoint
